...
Once the max Stream ID has been used (2^31-1), the Requester MAY re-use Stream IDs.
...
- The Responder MAY choose to ERROR[REJECT] any Stream ID if it consider the ID still in use. The Requester MAY retry on the next sequential Stream ID considered unused.
